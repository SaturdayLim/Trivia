...
| 33 | Repo visibility + GitHub Pages | OPEN (user) | Repo created PRIVATE as the safe default (pushed 2026-07-04) since `questions/archive-nsfw.md` would be publicly readable if public — and free GitHub Pages requires public on a Free plan. User AFK when asked; going public was judged to need explicit consent, not an AFK default. Decide: make public + enable Pages, or use a host that deploys from a private source (Netlify/Vercel/Cloudflare Pages) |
